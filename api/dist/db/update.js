...
async function getUserData(userName) {
  var url = "https://lichess.org/@/".concat(userName, "/perf/blitz");
  var userData;

  try {
    var res = await superagent.get(url).set('accept', 'application/vnd.lichess.v2+json');
...
exports.teamTallies = async () => {
  ...
  try {
    ...
    allTeams.filter(async k => {
      ...
      var totalHer = await user.sum('her', {
        where: {
          teamId: k.id
        }
      });
